<div class="content home">
    ...
    <aside>
        ...
        <article class="sitecontent boxaside">
            ...
            <ul>
                ...
                <li class="line"></li>
                <li class="post"><span><?= $Posts; ?></span> produtos</li>
    ...
    <section class="content_statistics">
        ...
        <section>
            <h1 class="boxsubtitle">Artigos Recentes: <small> <?= $userlogin['user_name']; ?></small></h1>

            <?php
            $read->ExeRead("ws_posts", "WHERE post_author = :user ORDER BY post_date DESC LIMIT 3","user={$userlogin['user_id']}");
            if ($read->getResult()):
        ...
        <section>
            ...
            <?php
            $read->ExeRead("ws_posts", "WHERE post_author = :user ORDER BY post_views DESC LIMIT 3","user={$userlogin['user_id']}");
            if ($read->getResult()):
                foreach ($read->getResult() as $re):
                    extract($re);
                    ?>
                    <article>
                        ...
                        <ul class="info post_actions">
                            ...
                            <li><a class="act_edit" href="painel2.php?exe=posts/update&postid=<?= $post_id; ?>" title="Editar">Editar</a></li>
