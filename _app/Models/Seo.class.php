 ...
 * 
 * @copyright (c) 2015, Wilson Azer Junior Promoville
 */
class Seo {
        ...
        switch ($this->File):
            ...
            case 'produto':
                ...
                if (!$ReadSeo->getResult()):
                    ...
                    $this->Data = [$post_title . ' - ' . SITENAME, $post_content, HOME . "/produto/{$post_name}", HOME . "/uploads/{$post_cover}"];
            ...
            case 'pesquisa':
                ...
                break;     
                     ...
                     //SEO:: PESQUISA Emprsea
            case 'pesquisa_empresa':
                 $ReadSeo->ExeRead("ws_users", "WHERE user_url = :link AND user_id <> 1", "link={$this->Link}");
                if (!$ReadSeo->getResult()):
                    $this->seoData = null;
                    $this->seoTags = null;
                else:
                    extract($ReadSeo->getResult()[0]);
                    $this->seoData = $ReadSeo->getResult()[0];
                    $this->Data = ["Pesquisa por: {$this->Link}" . ' - ' . SITENAME, "Sua pesquisa por {$this->Link} retornou {$this->seoData['count']} resultados!", HOME . "/pesquisa_empresa/{$this->Link}", INCLUDE_PATH . '/images/site.png'];
                endif;
                break;
                
                   //SEO:: Mais Vistos
            case 'maisvistos':
                $ReadSeo->ExeRead("ws_posts", "WHERE post_status = 1 ORDER BY post_views");
                if (!$ReadSeo->getResult()):
                    $this->seoData = null;
                    $this->seoTags = null;
                else:
                ...
                break;
                      ...
                      //SEO:: Mais Vistos
            case 'novas':
                $ReadSeo->ExeRead("ws_posts", "WHERE post_status = 1 ORDER BY post_id DESC");
                if (!$ReadSeo->getResult()):
                    $this->seoData = null;
                    $this->seoTags = null;
                else:
