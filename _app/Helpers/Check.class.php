...
class Check {
     ...
     * @return HTML = imagem redimencionada!
     */
    public static function FomataDuracao($tempo,$unidade) {
        ...
        switch ($unidade):
        case 'd':
            $unFormatada = date('Y-m-d H:i:s',  strtotime($timeHoje.$temp.'days'));
            break;
         case 's':
                 $unFormatada = date('Y-m-d H:i:s',  strtotime($timeHoje.$temp.'weeks'));
            break;
         case 'm':
                  $unFormatada = date('Y-m-d H:i:s',  strtotime($timeHoje.$temp.'months'));
            break;
        endswitch;
      ...
      public static function VerificaFimAnuncio() {

         $pst = new Read();
         $hoje = date('Y-m-d H:i:s');
         $pst->ExeRead("ws_posts","WHERE post_fim_an < :hoje","hoje={$hoje}");
         if(!$pst->getResult()):
         else:     
             foreach ($pst->getResult()as $desPubli):    
               $dados = array('post_status'=>0);
               $upAnuncio = new Update;
               $upAnuncio->ExeUpdate("ws_posts", $dados, "WHERE post_id = :id","id={$desPubli['post_id']}");
        
             endforeach;
         endif;
    }

     public static function VerificaPanoAnun($user,$plano) {

         $pst = new Read();
         $pst->FullRead("SELECT COUNT(post_author) AS cont FROM ws_posts WHERE post_author = :user", "user={$user}");
         $contador = $pst->getResult()[0];
         $c =  $contador['cont'];
